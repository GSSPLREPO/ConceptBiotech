...
namespace ConceptBiotech.WebAPI.Helper
{
    /// <summary>
    /// This class is used to add loggin to the API
    /// </summary>
    public class NLogger : ITraceWriter
    {
        ...
        private void Log(TraceRecord record)
        {
            ...
            if (record.Exception != null && !string.IsNullOrWhiteSpace(record.Exception.GetBaseException().Message))
            {
                ...
                {
                    var exception = record.Exception as Exception;
                    message.Append("").Append("Error: " + record.Exception.GetBaseException().Message + Environment.NewLine);
                    //if (exception.StackTrace != null)
                    //{
                    //    message.Append("").Append("Error: " + exception.StackTrace + Environment.NewLine);
                    //}
                }
            }
